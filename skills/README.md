...
26 original, batteries-included [Agent Skills](https://platform.claude.com/docs/en/agents-and-tools/agent-skills/overview) for putting the tools in this list to work. They auto-trigger when you ask your coding agent to add tracing, cut cost, debug an agent, and so on.
...
| ⚙️ [`validate-genai-spans`](validate-genai-spans/SKILL.md) | Lint spans against the OTel GenAI spec (tested). |
...
| ⚙️ [`estimate-llm-cost`](estimate-llm-cost/SKILL.md) | Price calls + project bills with a real calculator (tested). |
...
| ⚙️ [`check-answer-consistency`](check-answer-consistency/SKILL.md) | Reference-free hallucination signal from sample agreement (tested). |
...
| ⚙️ [`scrub-pii-from-text`](scrub-pii-from-text/SKILL.md) | Mask emails/cards/SSNs/IBANs before export, Luhn-checked (tested). |
